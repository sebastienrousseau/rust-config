...
%global npm_bare      rust-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-rust-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Rust configuration providing standardized rustfmt and clippy rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/rust-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/rust-config is a shareable configuration preset with no runtime
...
install -pm 0644 rustfmt.toml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 clippy.toml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
