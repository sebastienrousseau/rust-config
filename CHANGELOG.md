...
- Added a `native-validate` CI job that installs the real toolchain (rustfmt, clippy) and validates the shipped presets.
- Corrected the OpenSSF Best Practices badge, which pointed at another project.
## [0.0.1] - 2026-09-09

### Added

- Initial release of `@sebastienrousseau/rust-config` conforming to REPO-STANDARD.
- Standardized Rust configuration presets: `rustfmt.toml`, `clippy.toml`.
