...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/rust-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Pedantic Clippy Lints & Deterministic rustfmt](#pedantic-clippy-lints--deterministic-rustfmt) — before and after rule comparison
...
Install `@sebastienrousseau/rust-config` as a development dependency:
...
# pnpm
pnpm add -D @sebastienrousseau/rust-config

# yarn
yarn add -D @sebastienrousseau/rust-config

# bun
bun add -d @sebastienrousseau/rust-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/rust-config.git
cd rust-config
make            # check + test
```
...
- **Target environment**: Optimized for Rust 2021 / Clippy.
...
```bash
cp node_modules/@sebastienrousseau/rust-config/rustfmt.toml ./rustfmt.toml
cp node_modules/@sebastienrousseau/rust-config/clippy.toml ./clippy.toml
```

### Or point the tool at the installed file

```bash
cargo fmt -- --config-path node_modules/@sebastienrousseau/rust-config/rustfmt.toml
```
...
`@sebastienrousseau/rust-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/rust-config` solves this with three deliberate engineering choices:
...
## Pedantic Clippy Lints & Deterministic rustfmt

Enforces Rust 2021 edition idioms, documentation tests, explicit casts, and pedantic Clippy lints for maximum memory and type safety.

### Before (Unstandardized)

```text
# rustfmt.toml
# default unconfigured formatting
```

### After (@sebastienrousseau/rust-config Enforced)
...
When writing for experimental nightly-only compiler forks that break standard Clippy linter interfaces.
