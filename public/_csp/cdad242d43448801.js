...
lp=lm?'/rust-config/'+lm[1]:'/rust-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/rust-config/'+lm[1]+'/search-index.json':'/rust-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
